...
void FVM::Limiters::SuperbeeLimiter_t::calculate_derivatives(const std::vector<double> &x, const std::vector<double> &u, const std::vector<double> &a, const std::vector<double> &p, std::vector<double> &du_dx, std::vector<double> &da_dx, std::vector<double> &dp_dx) {
    ...
    for (int i = 1; i <= x.size() - 2; ++i) {
        ...
        du_dx[i] = std::copysign(1.0, a_u) * std::max(0.0, std::max(std::min(2 * std::abs(a_u), std::copysign(1.0, a_u) * b_u), std::min(std::abs(a_u), 2.0 * std::copysign(1.0, a_u) * std::abs(b_u))));
        da_dx[i] = std::copysign(1.0, a_a) * std::max(0.0, std::max(std::min(2 * std::abs(a_a), std::copysign(1.0, a_a) * b_a), std::min(std::abs(a_a), 2.0 * std::copysign(1.0, a_a) * std::abs(b_a))));
        dp_dx[i] = std::copysign(1.0, a_p) * std::max(0.0, std::max(std::min(2 * std::abs(a_p), std::copysign(1.0, a_p) * b_p), std::min(std::abs(a_p), 2.0 * std::copysign(1.0, a_p) * std::abs(b_p))));
    }
}
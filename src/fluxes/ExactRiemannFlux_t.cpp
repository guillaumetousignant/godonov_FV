...
FVM::Fluxes::ExactRiemannFlux_t::ExactRiemannFlux_t(int n_faces) :
        a_star_L_(n_faces),
        a_star_R_(n_faces),
        u_star_(n_faces),
        p_star_L_(n_faces),
...
void FVM::Fluxes::ExactRiemannFlux_t::calculate_fluxes(double delta_t, const std::vector<double> &gamma, const std::vector<double> &u, const std::vector<double> &a, const std::vector<double> &p, std::vector<double> &F_1, std::vector<double> &F_2, std::vector<double> &F_3) {
    for (int i = 0; i < F_1.size(); ++i) {
        ...
        C_R_[i] = FVM::RiemannProblem::calculate_C(a[i+1], p[i+1], gamma[i+1]);
        FVM::RiemannProblem::solve_flux(a[i], a[i+1], u[i], u[i+1], p[i], p[i+1], gamma[i], gamma[i+1], C_L_[i], C_R_[i], u_star_[i], a_star_L_[i], a_star_R_[i], p_star_L_[i], p_star_R_[i], p_star_prime_L_[i], p_star_prime_R_[i]);
        FVM::RiemannProblem::calculate_a_star(a[i], a[i+1], u[i], u[i+1], p[i], p[i+1], gamma[i], gamma[i+1], u_star_[i], a_star_L_[i], a_star_R_[i], p_star_L_[i], p_star_R_[i]);
        double a_face, u_face, p_face, gamma_face;
        FVM::RiemannProblem::get_boundary_state(a_face, u_face, p_face, gamma_face, delta_t, a[i], a[i+1], u[i], u[i+1], p[i], p[i+1], gamma[i], gamma[i+1], u_star_[i], a_star_L_[i], a_star_R_[i], p_star_L_[i], p_star_R_[i]);

...
void FVM::Fluxes::RoeEntropyFlux_t::calculate_fluxes_higher_order(double delta_t, const std::vector<double> x, const std::vector<double> &gamma, const std::vector<double> &u, const std::vector<double> &a, const std::vector<double> &p, std::vector<double> &F_1, std::vector<double> &F_2, std::vector<double> &F_3, const std::vector<double> du_dx, const std::vector<double> da_dx, const std::vector<double> dp_dx) {
    ...
    for (int i = 0; i < F_1.size(); ++i) {
        ...
        const double lambda_0_L = u_L - a_L;
        const double lambda_2_L = u_L + a_L;
        const double lambda_0_R = u_R - a_R;
        const double lambda_2_R = u_R + a_R;
        const double delta_0 = std::max(0.0, 4 * (lambda_0_R - lambda_0_L));
        const double delta_2 = std::max(0.0, 4 * (lambda_2_R - lambda_2_L));
...
void FVM::Fluxes::RoeEntropyFlux_t::calculate_fluxes_higher_order(double delta_t, FVM::Entities::Mesh2D_t &mesh) {

}

void FVM::Fluxes::RoeEntropyFlux_t::calculate_fluxes_higher_order_hat(double delta_t, FVM::Entities::Mesh2D_t &mesh) {
